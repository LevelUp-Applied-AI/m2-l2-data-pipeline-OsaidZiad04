...
def clean_data(df):
    ...
    df['unit_price'].fillna(df['unit_price'].median())
...
def create_visualizations(df, output_dir=OUTPUT_DIR):
    ...
    plt.xticks(rotation=45)
    plt.tight_layout()
    plt.savefig(f'{output_dir}/daily_revenue_trend.png', dpi=150, bbox_inches='tight')
    plt.close(fig)
    ...
    plt.tight_layout()
    plt.savefig(f'{output_dir}/avg_order_by_payment.png', dpi=150, bbox_inches='tight')
    plt.close(fig)
